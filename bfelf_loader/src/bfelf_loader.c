                        ...
                        struct bfelf_rela *rela)
{
    ...
    bfelf64_addr *ptr = (bfelf64_addr *)(ef->exec_addr + rela->r_offset);
...
private_process_segments(struct bfelf_file_t *ef)
{
    ...
    for (i = 0; i < ef->ehdr->e_phnum; i++)
    {
        ...
        switch (phdr->p_type)
        {
            ...
            default:
                return unsupported_file("unsupported segment type");
        }
    }
                     ...
                     void **addr)
{
    ...
    *addr = (void *)(ef->ehdr->e_entry + ef->exec_virt);
    return BFELF_SUCCESS;
}
...
private_process_dynamic_section(struct bfelf_file_t *ef)
{
        ...
        switch (dyn->d_tag)
        {
            ...
            case bfdt_needed:
                ...
                if (ef->num_needed >= BFELF_MAX_NEEDED)
                    return loader_full("increase BFELF_MAX_NEEDED");
            ...
            case bfdt_hash:
                ...
                ef->hash = (bfelf64_word *)(dyn->d_ptr + ef->exec_addr);
            ...
            case bfdt_strtab:
                ef->strtab = (char *)(dyn->d_ptr + ef->exec_addr);
                break;

            case bfdt_symtab:
                ef->symtab = (struct bfelf_sym *)(dyn->d_ptr + ef->exec_addr);
                break;

            case bfdt_rela:
                ef->relatab = (struct bfelf_rela *)(dyn->d_ptr + ef->exec_addr);
                break;
            ...
            case bfdt_syment:
                ...
                if (dyn->d_val != sizeof(struct bfelf_sym))
                    unsupported_file("unsupported symbol entry size");
            ...
            case bfdt_init_array:
                ef->init_array = dyn->d_ptr;
                break;

            case bfdt_fini_array:
                ef->fini_array = dyn->d_ptr;
                break;
    ...
    if (bind_now != 1)
        return unsupported_file("ELF files must be labeled bind_now");
    ...
    if (ef->strsz != 0)
        ef->symnum = (bfelf64_xword)ef->strtab - (bfelf64_xword)ef->symtab / ef->strsz;
                 ...
                 char *exec_virt)
{
    ...
    if (ef->dyntab != 0)
    {
        ret = private_process_dynamic_section(ef);
        if (ret != BFELF_SUCCESS)
            return ret;
    }

    if ((ef->flags_1 & bfdf_1_pie) != 0 || ef->library == 1)
